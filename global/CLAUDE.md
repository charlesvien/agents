...
- **Comments**: Default is zero comments. Code must be self-explanatory; if it needs a comment to be understood, it's too complex: simplify or rename until it doesn't. Never write comment blocks: no multi-line explanations, no JSDoc or docstring headers on ordinary code, no banners, no section dividers, no step-by-step narration, no comments restating what the code does. Never write comments addressed to me or a reviewer (what changed, why the change is correct, what used to be there): that context belongs in your reply or the PR text, not the file. Comments outlive the session; a stale or wrong one misleads every future reader, poisons agent context and is worse than no comment. A comment is a last resort: one line, only for a *why* the code can't express (non-obvious tradeoff, workaround, external constraint). This cap holds even in comment-heavy files: surrounding density is never a license to add more. In a file with no comments, add none.
